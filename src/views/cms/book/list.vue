  ...
  <div id="app-container">
    ...
    <el-table :data="bookList" border fit highlight-current-row>
      ...
      <el-table-column label="连载">
        <template slot-scope="scope">
          {{ scope.row.serial === 1 ? '是' : '否'}}
        </template>
      </el-table-column>
      <el-table-column prop="workNumber" label="字数" />
      <el-table-column label="状态">
        <template slot-scope="scope">
          {{ scope.row.state === 1 ? '上线' : '未上线'}}
        </template>
      </el-table-column>
...
<!--        </template>-->
      ...
      <el-table-column label="操作" align="center">
        <template slot-scope="scope">
          <el-button type="danger" size="mini" @click="deleteBook(scope.row.id, scope.row.state)">删除</el-button>
          <router-link :to="'/cms/book/update/'+scope.row.id">
            <el-button type="primary" size="mini" >修改</el-button>
          </router-link>
          <el-button v-if="scope.row.state === 0" type="danger" size="mini" icon="el-icon-delete" @click="upOrDownBook(scope.row.id, 1)">上架书籍</el-button>
